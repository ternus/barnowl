...
package BarnOwl::Complete::Zulip;

use BarnOwl::Completion::Util qw(complete_flags);
use BarnOwl::Module::Zulip;

# our %classes = ();
# our %realms = map {$_ => 1} qw(ATHENA.MIT.EDU ANDREW.CMU.EDU ZONE.MIT.EDU);
# our %users = ();

sub complete_class    { keys %BarnOwl::Module::Zulip::zulip_subs; }
sub complete_instance { }
sub complete_realm    { }
sub complete_opcode   { }
sub complete_user     { keys %BarnOwl::Module::Zulip::zulip_users; }

sub complete_zulipwrite {
    my $ctx = shift;
...
BarnOwl::Completion::register_completer("zulip:write"    => \&complete_zulipwrite);
